...
**节点特征基线痛点**：原demo节点特征仅由15种货币的进款均值、出款均值，以及bank列（LabelEncoder编码）构成（31维稀疏特征），存在严重的特征表达缺陷。
...
所有特征**在每个时序窗口内独立进行处理**
备注1. **金额、频次类特征**：先执行 `log1p()` 对数变换，再使用 `StandardScaler` 标准化处理
备注2. **比率、计数类特例**
    - `net_flow_ratio`：本身取值区间[-1,1]，跳过log1p，仅标准化
    - `unique_currency_count`：货币种类是小范围离散计数，无长尾分布，跳过log1p，仅标准化
备注3.交易金额统一货币单位为USD计算的金额，使用的汇率是交易数据中的汇率总位数。

**交易边特征基线痛点**：原交易边特征（6列）：时间戳、转入金额、转入货币、转出金额、转出货币、转账方式

(1)、转入货币、转出货币、转账方式使用LabelEncoder编码，引入了不存在的数值大小关系，破坏了权重计算与梯度更新。

(2)、转入金额、转出金额的货币没有统一，失去可比性，并且只进行了对数压缩。

(3)、时间戳使用MinMaxScaler，时间被缩放到[0,1],当有新交易时间戳大于训练期的时间最大值，则输入模型后归一化结果就会超过1导致特征分布漂移。

**交易边特征优化方案**：转成信息密集的8列+转账方式列

(1)、format_*：转账交易列使用热独编码

(2)、Amount_USD_log：交易金额统一成美元，保留转出金额这一列，并取对数log1p

(3)、hour_sin,hour_cos：时间戳的转为小时周期性编码、星期周期性编码，将0-23点映射至单位圆坐标，均取对数lop1p

(4)、dow_sin,dow_cos：将周一~周日映射至单位圆坐标,均取对数lop1p

(4)、delta_last_in_log、delta_last_out_log：账户本笔出款距离上一次进款/出款的秒数，并取对数，看连续分拆转账频率

(5)、is_cross_currency: 是否跨币种交易（1/0)
...
## 交易边特征基线痛点
原始交易边特征共6列：时间戳、转入金额、转入货币、转出金额、转出货币、转账方式
1. 转入货币、转出货币、转账方式直接使用`LabelEncoder`编码，人为引入不存在的数值大小关系，干扰图模型权重学习与梯度更新。
2. 转入、转出金额未统一币种，不同货币的交易规模不具备可比性，且仅做简单对数压缩。
3. 时间戳直接使用`MinMaxScaler`归一化到`[0,1]`区间。当测试集新交易时间晚于训练集最大时间，归一化后数值会大于1，引发特征分布漂移。

## 交易边特征优化方案
重构为信息更密集的边特征，一共8维（转账方式采用独热编码）
1. **format_***：转账方式字段，使用One‑Hot独热编码，规避类别特征产生虚假大小关系
2. **is_cross_currency**：是否为跨币种交易，0/1二值特征
3. **Amount_USD_log**：交易金额统一换算为美元后，执行`log1p`对数变换，消除币种差异、压缩金额长尾极值
4. **hour_sin、hour_cos**：小时周期编码。将0‑23小时映射到单位圆，保留时间周期性，解决跨零点断层问题
5. **dow_sin、dow_cos**：星期周期编码。将周一~周日映射到单位圆，保留星期周期规律，解决跨周断层问题
6. **delta_last_in_log**：本笔交易距离账户**上一笔转入**的时间间隔(秒)，`log1p`变换，捕捉资金流入频率
7. **delta_last_out_log**：本笔交易距离账户**上一笔转出**的时间间隔(秒)，`log1p`变换，识别短时间过桥快转、连续拆分交易行为

### 🔀 交易边特征工程优化 (Edge Features Optimization)

#### ❌ 原交易边基线痛点 (Baseline Limitations)
原始数据中的 6 维交易边特征（时间戳、转入金额、转入货币、转出金额、转出货币、转账方式）存在严重的工程与业务缺陷：
1. **编码逻辑缺陷**：货币与转账方式使用 `LabelEncoder` 编码，硬生生引入了无意义的数值大小关系，干扰 GATv2 注意力权重的计算与梯度更新。
2. **币种数值失真**：转入/转出金额未统一汇率，导致跨币种数值（如 JPY 与 USD）失去可比性，且仅做对数处理，缺乏尺度归一。
3. **时间外推性失效**：绝对时间戳使用 `MinMaxScaler` 归一化，当推理期出现超出训练集最大时间的新交易时，归一化值将超过 $1.0$，引发特征分布漂移（Data Drift）。

---

#### 💡 交易边特征优化方案 (Redesign & Normalization)
重构为信息更加稠密且符合业务物理逻辑的边特征矩阵（独热编码列 + 7 维数值/周期特征）：

| 序号 | 特征名称 (Feature) | 预处理与编码策略 | 风控业务含义 / 解决痛点 |
| :---: | :--- | :--- | :--- |
| **1** | `format_*` | **One-Hot 独热编码** | 消除 `LabelEncoder` 的人工偏置，独立表征 ACH、Wire、Cheque 等渠道的风控权重。 |
| **2** | `is_cross_currency` | **二值特征 (0/1)** | 标识是否属于跨币种/跨境交易，直接捕捉高危洗钱通道。 |
| **3** | `Amount_USD_log` | **汇率统一 (USD) + $\log1p$** | 将交易金额统一换算为美元并做对数平滑，消除币种差异，还原真实的资金规模。 |
| **4** | `hour_sin`, `hour_cos` | **正弦/余弦周期编码 ($\sin/\cos$)** | 将 0-23 点映射至单位圆坐标，消除跨日（23点到0点）数值断层，解决时间外推漂移问题。 |
| **5** | `dow_sin`, `dow_cos` | **正弦/余弦周期编码 ($\sin/\cos$)** | 将周一至周日映射至单位圆坐标，消除跨周数值断层，捕捉周末/夜间洗钱规律。 |
| **6** | `delta_last_out_log` | **相对秒数 + $\log1p$** | 当前出款距离账户**上一次出款**的时间差（秒），识别自动化高频拆单与定时连环转账。 |
| **7** | `delta_last_in_log` | **相对秒数 + $\log1p$** | 当前出款距离账户**上一次进款**的时间差（秒），精准捕捉“即进即出”的极速过桥中转洗钱模式。 |

> **数据标准化补充说明：**
> 上述所有连续数值特征（`Amount_USD_log` 及 `delta_*` 时间差特征）在输入 GATv2 模型前，均统一进行了 **Z-Score 标准化 (StandardScaler)**，将特征缩放至均值为 0、标准差为 1 的分布，确保边注意力（Edge Attention）的稳定收敛。
> 
3、缺乏对样本数据不平衡的处理
