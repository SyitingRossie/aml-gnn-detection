...
1. 损失函数的正样本加权：BCEWithLogitsLoss设置pos_weight权重，放大对正样本猜错的损失，强迫模型必须重视极少数的洗钱客户。
2. 评估指标选择：放弃了容易受负样本主导的 ROC-AUC，转而以 PR-AUC (Average Precision) 为核心监控指标。
3. 动态阈值选取：由于反洗钱业务更倾向于‘宁可错杀、不能放过’，因此在验证集上通过最大化$F_beta$-Score(给Recall赋以N倍于Precision的权重)来动态搜索最佳决策阈值。
