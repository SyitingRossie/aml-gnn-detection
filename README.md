...
2. 动态阈值选取：由于反洗钱业务更倾向于‘宁可错杀、不能放过’，因此在验证集上改用针对极度不平衡场景的PR-AUC选型指标和最大化$F_beta$-Score(给Recall赋以N倍于Precision的权重)来动态搜索最佳决策阈值,平衡抓出率与人工复核成本。
...
2. 评估指标选择：放弃了容易受负样本主导的ROC-AUC，以PR-AUC(Average Precision))为核心监控指标。
